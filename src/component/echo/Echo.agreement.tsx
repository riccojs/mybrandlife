...
import { useToggleEchoMutation } from "../../redux/features/echo/echoApi";
...
function EchoAgreement({ isShow, setShow, status, body }: DataTypes) {
  useBodyScroll(isShow);
  const { user } = useAuth();
  const id = user?.id;
  const [toggleEvent, { isLoading }] = useToggleEchoMutation();

  const handleSubmit = () => {
    const echo = { status: status };
    toggleEvent({ id, echo })
      .unwrap()
      .then((res) => {
